...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      hr(),
      p("This application displays spatial insect patterns in Ecuador, integrating conservation areas, registries, and natural regions.")
    ),
    mainPanel(
      tabsetPanel(
        tabPanel(
...
server <- function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    # Compute initial palette for the default grid variable "Ttl_rcr"
    var_data <- grid_shp[["Ttl_rcr"]]
    ...
    if (length(clean_data) > 1) {
      ...
      init_pal <- colorBin("Blues", domain = var_data, bins = breaks, na.color = "transparent")
    } else {
      init_pal <- colorNumeric("Blues", domain = var_data, na.color = "transparent")
    }
    ...
    leaflet(records_sf) %>%
      ...
      addPolygons(
        data = grid_shp, weight = 1, color = "#2c3e50", fillColor = ~ init_pal(var_data), fillOpacity = 0.6,
        group = "Sampling Grid (10km)",
        popup = ~ paste0(
          "<strong>Grid Cell ID:</strong> ", id, "<br>",
          "<strong>Total Records:</strong> ", Ttl_rcr
        )
      ) %>%
      addLayersControl(
